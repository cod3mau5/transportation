...
function daisy_blog_register_popular_post_widget() {
    register_widget( 'Graphthemes_Widget_Popular_Post' );
}
add_action( 'widgets_init', 'daisy_blog_register_popular_post_widget' );
...
class Graphthemes_Widget_Popular_Post extends WP_Widget {
    ...
    function __construct(){
        if( ! is_customize_preview() ) add_action('wp', array( $this, 'daisy_blog_set_views' ) );
        
        parent::__construct(
            'daisy_blog_widget_popular_post', // Base ID
            esc_html__( 'Graphthemes: Popular Post', 'daisy-blog' ), // Name
            array( 'description' => esc_html__( 'A Popular Post Widget', 'daisy-blog' ), ) // Args
        );
    }
     ...
     */
    function daisy_blog_set_views( $post_id ) {
        if ( in_the_loop() ) {
            $post_id = get_the_ID();
          } 
        else {
     ...
     */
    function daisy_blog_get_views( $post_id ){
        $count_key = '_daisy_blog_view_count';
        $count = get_post_meta( $post_id, $count_key, true );
        if( $count == '' ){        
            return __( "0 View", 'daisy-blog' );
        }elseif($count<=1){
            return $count. __(' View', 'daisy-blog' );
        }else{
            return $count. __(' Views', 'daisy-blog' );    
        }    
    }
    ...
    public function widget( $args, $instance ) {
        ...
        $title          = ! empty( $instance['title'] ) ?  $instance['title'] : __( 'Popular Posts', 'daisy-blog' );
                    ...
                    <li>
                        ...
                        <div class="entry-header">
                            ...
                            <?php if( $based_on == 'views' && $view_count ){ ?>
                                <span class="view-count"><?php echo esc_html( $this->daisy_blog_get_views( get_the_ID() ) );?></span>
                            <?php }
    ...
    public function form( $instance ) {
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'num_post' ) ); ?>"><?php esc_html_e( 'Number of Posts', 'daisy-blog' ); ?></label> 
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'based_on' ) ); ?>"><?php esc_html_e( 'Popular based on:', 'daisy-blog' ); ?></label>
            <select id="<?php echo esc_attr( $this->get_field_id( 'based_on' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'based_on' ) ); ?>" class="based-on">
                <option value="views" <?php selected( $based_on, 'views' ); ?>><?php esc_html_e( 'Post Views', 'daisy-blog' ); ?></option>
                <option value="comments" <?php selected( $based_on, 'comments' ); ?>><?php esc_html_e( 'Comment Count', 'daisy-blog' ); ?></option>
            </select>
        </p>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_thumbnail' ) ); ?>"><?php esc_html_e( 'Show Post Thumbnail', 'daisy-blog' ); ?></label>
        </p>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_postdate' ) ); ?>"><?php esc_html_e( 'Show Post Date', 'daisy-blog' ); ?></label>
        </p>

        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'target' ) ); ?>">
            <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'target' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'target' ) ); ?>" type="checkbox" value="1" <?php echo checked($target,1);?> /><?php esc_html_e( 'Open in New Tab', 'daisy-blog' ); ?> </label>
        </p>
    ...
    public function update( $new_instance, $old_instance ) {
        ...
        $instance = array();
        $instance['title']          = ! empty( $new_instance['title'] ) ? sanitize_text_field( $new_instance['title'] ) : __( 'Recent Posts', 'daisy-blog' );
